...
export const Products1 = () => {
    ...
    const [prodmainidtodelete, setProdmainidtodelete] = useState(0);
                    ...
                    <div className="dialog-content">
                        ...
                        <div className="form-group">
                            ...
                            <textarea
                                ...
                                value={prod.details}
                                onChange={e => { setProd({ ...prod, details: e.target.value.substring(0, 200) }); }}
            ...
            {showDelete && (
                <dialog className="edit-dialog delete-dialog" ref={refDialog}>
                    ...
                    <div className="dialog-content">
                        ...
                        {/* Get branches that have this product */}
                        {(() => {
                            // Find branches that have this product
                            const branchesWithProduct = [];
                            if (prod && prod.productsMains && prod.productsMains.length > 0) {
                                // Get unique branch IDs that have available instances of this product
                                const branchIds = [...new Set(
                                    prod.productsMains
                                        .filter(pm => pm.available && pm.canBeUsed)
                                        .map(pm => pm.idSnif)
                                )];
                                // Get the full branch objects
                                branchIds.forEach(branchId => {
                                    const branch = sniffim.find(s => s.id === branchId);
                                    if (branch) {
                                        branchesWithProduct.push(branch);
                                    }
                                });
                            }

                            if (branchesWithProduct.length === 0) {
                                return (
                                    <div className="empty-branches-message">
                                        <div className="no-branches-icon">
                                            <StoreIcon style={{ fontSize: '3rem', color: '#ddd' }} />
                                        </div>
                                        <p>המוצר אינו קיים באף סניף</p>
                                    </div>
                                );
                            }
                            ...
                            return (
                                <div className="branches-list">
                                    ...
                                    {branchesWithProduct.map((s) => (
                                        <div
                                            className={`branch-item ${s.id === snif.id ? 'selected' : ''}`}
                                            onClick={() => {
                                                setSnif(s);
                                                // עדכון מלא של prodMain עם פרטי הסניף הנוכחי
                                                setProdMain({
                                                    ...prodMain,
                                                    idSnif: s.id,
                                                    id: 0 // איפוס בחירת המופע
                                                });
                                                console.log("בחרת סניף:", s.name, "ID:", s.id);
                                            }}
                                            key={s.id}
                                        >
                                            <StoreIcon className="branch-icon" />
                                                ...
                                                <div className="product-instances">
                                                    <div className="form-group">
                                                        ...
                                                        <select
                                                            ...
                                                            className="form-input"
                                                            onChange={(e) => {
                                                                setProdmainidtodelete(parseInt(e.target.value));
                    ...
                    <div className="dialog-actions">
                        ...
                        <button
                            ...
                            onClick={() => {
                                dispatch(DeleteProdMainThunk(prodmainidtodelete));
                                setShowDelete(false);
                            }}
                            disabled={!prodmainidtodelete}
                        >
                            הסר מוצר מהסניף
                        </button>
                    </div>
